...
fun drawRoute(
    ...
    onRouteDrawn: (List<LatLng>, String) -> Unit
) {
    if (currentLocation == null || markedLocation == null) {
        showAlertDialog(
            context = context,
    ...
    val destination = "${markedLocation.latitude},${markedLocation.longitude}"
            ...
            if (response.isSuccessful) {
                ...
                directionsResponse?.let {
                    if (it.routes.isNotEmpty()) {
                        ...
                        onRouteDrawn(polylinePoints, routeInfo)
                    } else {
                        showAlertDialog(
                            context = context,
